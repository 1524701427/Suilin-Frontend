<template>
  <view class="page">
    ...
    <view class="card">
      ...
      <picker :range="relations" @change="onRelationChange"><view class="picker">{{form.relation || '请选择关系'}}</view></picker>

      <text class="label">出生日期（可选）</text>
      <picker mode="date" :value="form.birthday" @change="onBirthdayChange"><view class="picker">{{form.birthday || '未填写，不会显示年龄'}}</view></picker>
      ...
      <view class="tags"><view v-for="tag in healthOptions" :key="tag" class="tag" :class="{active:form.healthTags.includes(tag)}" @tap="toggleTag(tag)">{{tag}}</view></view>
...
export default {
  data(){return{relations:['爸爸','妈妈','爷爷','奶奶','外公','外婆','其他'],healthOptions:['高血压','糖尿病','心脏疾病','其他慢病'],form:{name:'',relation:'',birthday:'',phone:'',healthTags:[]},loading:false}},
  methods:{
    onRelationChange(e){this.form.relation=this.relations[Number(e.detail.value)]},
    onBirthdayChange(e){this.form.birthday=e.detail.value},
    toggleTag(tag){const i=this.form.healthTags.indexOf(tag);if(i>=0)this.form.healthTags.splice(i,1);else this.form.healthTags.push(tag)},
    async save(){
      ...
      this.loading=true
      try{
        const data=await elderApi.create({...this.form,name:this.form.name.trim(),birthday:this.form.birthday||null,phone:this.form.phone||null})
