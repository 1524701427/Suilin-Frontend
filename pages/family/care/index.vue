<template>
  <view class="page">
    ...
    <view class="hero"><text class="eyebrow">家庭照护</text><text class="hero-title">把每天的事情安排清楚</text><text class="hero-desc">这里只展示家人实际创建的提醒，不使用写死数据。</text></view>
    <view v-if="loading" class="empty-card"><text>正在加载...</text></view>
    <view v-else-if="!elders.length" class="empty-card"><text class="empty-title">还没有长辈资料</text><text class="empty-desc">先到“我的”添加长辈，再设置提醒。</text><button class="primary" @tap="addElder">添加长辈</button></view>
    <template v-else>
      ...
      <view class="toolbar"><view><text class="toolbar-title">{{selected?.name}}的提醒</text><text class="toolbar-desc">{{reminders.length}} 条记录</text></view><button class="add-btn" @tap="addReminder">+ 新建提醒</button></view>
      <view v-if="reminders.length" class="list">
        <view v-for="r in reminders" :key="r.id" class="card" @tap="openReminder(r)">
          <view class="icon">{{iconFor(r.type)}}</view><view class="main"><text class="title">{{r.title}}</text><text class="meta">{{r.scheduleTime}}{{r.dosage?' · '+r.dosage:''}}</text><text class="source">由家人创建 · {{r.enabled?'已启用':'已停用'}} · 点击管理</text></view>
        </view>
      </view>
